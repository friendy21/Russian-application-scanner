import { db, type Carton, type Scan, type VerificationEvent } from './dexie';
...
export async function exportAllJson(): Promise<string> {
    const [cartons, scans, verification_events] = await Promise.all([
        db.cartons.toArray(),
        db.scans.toArray(),
        db.verification_events.toArray(),
    ]);
    return JSON.stringify({ cartons, scans, verification_events, exported_at: new Date().toISOString() }, null, 2);
}

export async function importFromJson(jsonStr: string): Promise<void> {
    const data = JSON.parse(jsonStr);
    await db.transaction('rw', db.cartons, db.scans, db.verification_events, async () => {
        if (data.cartons?.length) {
            await db.cartons.bulkPut(data.cartons);
        }
        if (data.scans?.length) {
            await db.scans.bulkPut(data.scans);
        }
        if (data.verification_events?.length) {
            await db.verification_events.bulkPut(data.verification_events);
        }
    });
}
